#!/usr/bin/env bash
# ==============================================================================
# Jetson Video Player (Python 이전 버전) Installer — 명령 이름: jetson-player-py
#   (C++/Qt 버전은 저장소 최상위의 ./install.sh 로 설치하며 jetson-player 명령을 씁니다)
...
SCRIPTS_DIR="$(cd "$PROJECT_DIR/../../scripts" && pwd)"   # AI 설치 스크립트는 두 버전이 함께 씁니다
chmod +x "$PY_SCRIPT" "$BIN_SOURCE" "$SCRIPTS_DIR"/*.sh
...
if [ "$SET_DEFAULT" -eq 1 ]; then
    if command -v xdg-mime >/dev/null 2>&1; then
        ...
        for t in "${TYPES[@]}"; do
            [ -n "$t" ] && xdg-mime default jetson-player-py.desktop "$t"
        done
    else
...
echo ""
echo "   jetson-player-py /경로/동영상파일.mp4"
echo "   jetson-player-py /경로/동영상폴더/"
echo ""
case ":$PATH:" in
    *":$TARGET_DIR:"*) ;;
...
echo ""
echo "🤖 (선택) AI 자막 생성:        $SCRIPTS_DIR/setup_whisper.sh"
echo "🌐 (선택) AI 자막 한국어 번역:  $SCRIPTS_DIR/setup_translator.sh"
echo "=============================================================================="
