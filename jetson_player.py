...
def optimize_gstreamer_ranks():
    ...
    if hw_decoder:
        ...
        for name in sw_converters:
            ...
            if elem:
                elem.set_rank(Gst.Rank.NONE)
        print("⚡ [하드웨어 가속] Jetson NVMM 하드웨어 디코더 랭크 최상위 적용 완료.")
    else:
...
class JetsonSignageFlexiblePlayer(Gtk.Window):
    ...
    def build_playlist(self):
        ...
        if os.path.isdir(abs_path):
            ...
            extensions = ['*.webm', '*.mp4', '*.mkv', '*.mov', '*.avi', '*.ts', '*.wmv', '*.m4v', '*.flv']
            found_files = []
            for ext in extensions:
                found_files.extend(glob.glob(os.path.join(abs_path, ext)))
                found_files.extend(glob.glob(os.path.join(abs_path, ext.upper())))
            
            # 중복 제거 및 정렬
            self.playlist = sorted(list(set(found_files)))
    ...
    def on_realize(self, widget):
        ...
        # Window XID 획득 및 미리 캐싱 (GStreamer 스레드 간 동기화 안정성 제어)
    ...
    def play_current_video(self):
        ...
        # Jetson 하드웨어 가속 비디오 싱크 빈 구축 (nvvidconv + nveglglessink)
        # nvvidconv가 nveglglessink와 NVMM 버퍼 포맷(NV12, P010_10LE 등)을 유연하게 자동 협상합니다.
        vsink_desc = "nvvidconv ! nveglglessink sync=true"
        try:
    ...
    def on_bus_message(self, bus, message):
        ...
        if message.type == Gst.MessageType.EOS:
            self.error_count = 0  # 성공적 재생 시 에러 카운트 리셋
            ...
            if self.error_count >= max_allowed_errors:
                print("🛑 연속 에러 발생 횟수가 초과되어 폭주 방지를 위해 재생을 중단합니다.")
                if self.pipeline:
                    self.pipeline.set_state(Gst.State.NULL)
                return

            # 다음 비디오 시도 (약간의 지연 부여)
            GLib.timeout_add(100, self.play_next_video)
    ...
    def seek_relative(self, offset_seconds):
        ...
        dur_success, duration = self.pipeline.query_duration(Gst.Format.TIME)
        if dur_success and duration > 0 and target_ns >= duration:
            print("⏩ 탐색 위치가 영상 길이를 초과하여 다음 영상으로 넘어갑니다.")
            self.play_next_video()
            return
    ...
    def toggle_play_pause(self):
        ...
        success, state, pending = self.pipeline.get_state(100 * Gst.MSECOND)
        if success in (Gst.StateChangeReturn.SUCCESS, Gst.StateChangeReturn.ASYNC):
            current_or_pending = pending if pending != Gst.State.VOID_PENDING else state
            if current_or_pending == Gst.State.PLAYING:
                self.pipeline.set_state(Gst.State.PAUSED)
                print("⏸ 일시 정지")
            elif current_or_pending == Gst.State.PAUSED:
                self.pipeline.set_state(Gst.State.PLAYING)
                print("▶ 다시 재생")
    ...
    def play_next_video(self):
        ...
        if self.is_single_file_mode:
            if self.pipeline:
                res = self.pipeline.seek_simple(Gst.Format.TIME, Gst.SeekFlags.FLUSH | Gst.SeekFlags.KEY_UNIT, 0)
                if not res:
