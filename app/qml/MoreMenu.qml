...
JMenu {
    ...
    // 1) 지금 영상
    JMenuItem { text: "🔖 북마크 목록 (Ctrl+B)"; enabled: menu.hasVideo; onTriggered: App.requestDialog("bookmarks", "") }
    JMenuItem { text: "➕ 현재 위치 북마크 (B)"; enabled: menu.hasVideo; onTriggered: App.addBookmark() }
    ...
    // 2) 재생·화면·소리 설정
    JMenuItem { text: "🌙 야간 모드: 대사 크게·폭음 작게 (E)"; checkable: true; checked: App.nightMode; onTriggered: App.toggleNightMode() }
    JMenu {
    ...
    JMenuItem { text: "🔈 HDMI 패스스루 (AC3/DTS 원음 → AV 리시버, 실험적)"; checkable: true; checked: App.settings.audio_passthrough; onTriggered: App.togglePassthrough() }
    JMenu {
        title: "🤖 AI 자막 설정"
        JMenu {
            title: "인식 언어"
        ...
        JMenuItem { text: "영어로 번역하며 인식"; checkable: true; checked: App.settings.whisper_translate; onTriggered: App.toggleSetting("whisper_translate") }
        JMenuItem { text: "YouTube 영상은 자동 생성"; checkable: true; checked: App.settings.youtube_auto_ai_subtitles; onTriggered: App.toggleSetting("youtube_auto_ai_subtitles") }
        JMenuItem { text: "AI 자막을 만들면 자동으로 번역"; checkable: true; checked: App.settings.whisper_auto_translate; onTriggered: App.toggleSetting("whisper_auto_translate") }
        JMenu {
            title: "번역할 언어"
            Repeater {
    ...
    // 3) 도구 / 정보
    JMenuItem { text: "🌐 네트워크 폴더 열기 (SMB/NFS)..."; onTriggered: App.requestDialog("network", "") }
    JMenuItem { text: "📱 스마트폰 웹 리모컨..."; onTriggered: App.requestDialog("remote", "") }
    JMenuItem { text: "ℹ️ 미디어 정보 HUD (I)"; checkable: true; checked: App.hudVisible; onTriggered: App.toggleHud() }
    JMenuItem { text: "📌 항상 위에 표시 (T)"; checkable: true; checked: App.keepAbove; onTriggered: App.toggleKeepAbove() }
    JMenuItem { text: "🗗 미니 플레이어 (W)"; onTriggered: App.toggleMiniPlayer() }
    JMenuItem { text: "📄 로그 파일 보기"; onTriggered: App.openLogFile() }
